...
my_object_list = ['headphone','cellphone','scissors', 'watch','chandelier','octopus', 'snoopy','panda','chair', 'lamp']
...
def classify_svm(img):
    ...
    probabilities = clf.predict_proba(features)[0]  # Get probabilities for each class
    
    # Get indices of top 3 probabilities
    top3_indices = np.argsort(probabilities)[-3:]
    
    # Display top 3 predictions on the image with their probabilities
    font = cv2.FONT_HERSHEY_SIMPLEX
    for i, index in enumerate(reversed(top3_indices)):  # Reverse to show in descending order
        text = f"{clf.classes_[index]}: {probabilities[index]:.2f}"
        cv2.putText(img, text, (15, 25 + i*30), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
    return img
...
if __name__ == '__main__':
    vggfile = 'vgg_features_fc1.npz'.format(cnn_codes)
    ...
    if os.path.exists(vggfile):
        ...
        labels = npzfile['labels'] #9000+ labels. count per image
